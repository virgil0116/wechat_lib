...
module WechatLib
  ...
  class Client
    ...
    # 对于多用户微信营销平台的对接，需要把每次的expired_at和access_token保存在Redis中，每次使用则可以从Redis中获取expired_at和access_token，即
    # @client = WechatLib::Client.new(appid, appsecret, expired_at, access_token)
    # 获取access_token，则仍是：@client.get_access_token来获取
